...
if [ "$MODE" = "k3s" ]; then
  ...
  echo "  • Open WebUI (AI Chat):http://chat.ilcms.local (Browser AI Interface to Ollama)"
  ...
  echo "    $KUBECTL_CMD port-forward svc/ilcms-web -n ilcms 3000:3000    (Direct web access)"
  echo "    $KUBECTL_CMD port-forward svc/open-webui -n ilcms 3080:8080   (Direct Open WebUI access)"
  echo ""
  echo "  DNS Setup (for Traefik Ingress):"
  echo "    Add to /etc/hosts: 127.0.0.1 ilcms.local auth.ilcms.local chat.ilcms.local"
  echo ""
  echo "================================================================================"
  exit 0
...
echo "  • Open WebUI (AI Chat):http://localhost:3080 (Browser AI Interface to Ollama)"
